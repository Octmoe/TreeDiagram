...
- 根据用户意图选择“新设计”或“已有复杂设计重建”模式；
- 新设计从用户材料和对话中逐步提炼目标、约束与假设，并保持根部精简；
- 重建模式先盘点来源和总体边界，再按父级优先执行总体拆分、细节拆分、缺失推理链重建与整树问题复核；
- 直接来源事实、已有结论和 Agent 重建假设必须使用不同的认知与复核状态，不得把推测伪装成事实；
- 使用 `contradicts`、`violates`、`causes`、`amplifies`、`mitigates`、`reveals` 表达问题冲突、产生、加剧、缓解和发现路径；
- 信息模糊时直接在宿主聊天中询问；
- 在同一活动 ChangeSet 中按源节点小批提案并反复读取投影树，而非一次提交不可审阅的扁平节点堆；
- 根部确认仍要求显式用户操作。
...
用户可在 Sidecar 中显式关闭当前项目服务，数据保持不变；也可在输入完整项目名并核对绝对路径后执行
“归档并清空”。清空不做物理删除或目录重命名：Sidecar 先使用 SQLite 在线备份把一致快照写入项目内
`.treediagram-archive/`，再在版本未变化的前提下事务性清空活动数据库。这样不会被其它 MCP 连接持有的
Windows 文件句柄阻塞。只有归档和清空都完成后界面才显示成功，并展示可复制路径；永久删除只能由用户
在文件系统中手动完成。后续 Agent 使用项目时，Hook 会复用该空 workspace 并启动 Sidecar。
