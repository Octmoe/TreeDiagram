...
“归档并清空”。清空不做物理删除，而是在 Sidecar 退出、文件句柄释放后，由独立本地助手将整个
`.treediagram` 原子移动到项目内 `.treediagram-archive/`。界面必须把该路径展示并允许复制；永久删除只
能由用户在文件系统中手动完成。后续 Agent 使用项目时，Hook 可重新初始化空 workspace 并启动 Sidecar。
