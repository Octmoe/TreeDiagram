...
三栏之间的两条竖向分隔线可以拖动，分别调整左/中栏和中/右栏宽度；聚焦分隔线后也可用左右方向键微调。当前比例保存在浏览器中，刷新后继续使用；双击任一分隔线恢复默认比例。窄屏下界面会自动改为纵向排列，不显示分隔线。
...
候选较多且已经整体审查完成时，可以使用右栏的 **一键批准全部候选**。它不是简单地按列表顺序或树深从上到下循环点击，而会先为整个 ChangeSet 建立依赖图：新子节点依赖其 `contains` 父节点，其它关系依赖两个端点节点；随后执行拓扑排序，并把子节点与它唯一的 `contains` 放在同一采用步骤中。用户需要在弹窗中再次确认。

批量批准是单个数据库事务。存在父子环、缺失端点、多个 `contains` 父节点、同一实体上的竞争候选或关系引用本批次待移除节点时，整批都会拒绝并显示阻塞原因，不会留下“前半棵树已经批准”的状态。逐项批准入口仍然保留，适合需要逐条判断的候选。
...
启动器会同时校验插件版本和已构建运行时内容指纹。源码或本地插件运行时更新后，即使 SemVer 未变化，下一次任务启动、恢复或 MCP 初始化也会自动替换旧 Sidecar，避免新界面继续连接旧领域逻辑。
...
- **归档并清空工作区**：先显示将要创建的绝对归档路径，并要求输入完整项目名称。确认后，Sidecar 使用 SQLite 在线备份把可恢复的完整状态写入 `<项目>/.treediagram-archive/<时间-工作区标识>/`，再在事务中原地清空活动工作区并退出。即使其它 Codex 任务仍打开着同一数据库，也不需要重命名被 Windows 占用的文件；下次启动会打开空工作区。

“归档并清空”只有在在线备份和清空事务都成功后才显示完成页，不会把后台失败误报为成功。归档不会永久删除历史；完成页会再次显示归档路径。如需彻底删除，必须由用户稍后前往该路径手动删除。TreeDiagram 不提供远程或一键永久删除，以避免误操作。
